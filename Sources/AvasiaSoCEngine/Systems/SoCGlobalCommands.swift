...
enum SoCGlobalCommands {
    ...
    static let objectiveTriggers = ["OBJECTIVES", "OBJECTIVE", "JOURNAL", "QUEST"]

    static func isEatCommand(_ input: ParsedInput) -> Bool {
        eatTriggers.contains { input.contains($0) }
    }
    ...
    static func handle(_ input: ParsedInput, state: inout SoCGameState) -> [StyledLine]? {
        ...
        if helpTriggers.contains(where: { input.contains($0) }) {
            return [
                .hint("INVENTORY, OBJECTIVES, EAT, SAVE, TROPHY"),
                .hint("Move with NORTH, EAST, SOUTH, WEST, LOOK, TALK…")
            ]
        }

        if objectiveTriggers.contains(where: { input.contains($0) }) {
            return SoCJournal.objectiveLines(for: state)
        }
    ...
    static func eatInCombat(_ input: ParsedInput, state: inout SoCGameState) -> [StyledLine] {
        eat(input, state: &state)
    }
    ...
    private static func matchingFood(in input: ParsedInput, state: SoCGameState) -> SoCItem? {
        ...
        for item in SoCItem.allCases {
            ...
            if norm.contains(key) || norm.contains(item.rawValue.uppercased()) {
                return item
            }
            if item == .fieldRations && norm.contains("RATION") { return item }
        ...
        if norm == "EAT" || norm == "DRINK" {
            return SoCItem.allCases.first { ($0.healAmount != nil) && state.inventory[$0, default: 0] > 0 }
        }
        return nil
    }
}
